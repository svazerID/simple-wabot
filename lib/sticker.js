/*─────────────────────────────────────────
  ...
  Sticker creator with fluent-ffmpeg
─────────────────────────────────────────*/
...
export async function writeExif(media, metadata) {
  ...
  if (metadata && Object?.keys(metadata).length !== 0) {
    ...
    const json = {
      'sticker-pack-id': metadata?.packId || `ayanaMD-${Date.now()}`,
      'sticker-pack-name': metadata?.packName || global.packname || 'ayanaMD',
      'sticker-pack-publisher': metadata?.packPublish || global.author || 'KennDev',

...
export function smsg(conn, m, store) {
  ...
  // Unwrap view-once / document-with-caption
  ...
  m.body   = m.message?.conversation
    || msgContent?.text
    || msgContent?.caption
    || (m.mtype === 'listResponseMessage' ? msgContent?.singleSelectReply?.selectedRowId : '')
    || (m.mtype === 'buttonsResponseMessage' ? msgContent?.selectedButtonId : '')
    || (m.mtype === 'templateButtonReplyMessage' ? msgContent?.selectedId : '')
    || '';
  m.text   = m.body;
  ...
  // Media helpers
  m.mediaType = ['imageMessage','videoMessage','audioMessage','stickerMessage','documentMessage'].includes(m.mtype)
    ? m.mtype : null;
  ...
  if (ctx?.quotedMessage) {
    ...
    m.quoted.mediaType = ['imageMessage','videoMessage','audioMessage','stickerMessage','documentMessage']
      .includes(m.quoted.mtype) ? m.quoted.mtype : null;
  ...
  m.reply = (text, options = {}) =>
    conn.sendMessage(m.chat, { text: String(text), ...options }, { quoted: m });
...
export function bindConnMethods(conn, store) {
  ...
  conn.reply = (jid, text, quoted, opts = {}) =>
    conn.sendMessage(jid, { text: String(text), ...opts }, { quoted });

  conn.sendImage = (jid, buffer, caption = '', quoted, opts = {}) =>
    conn.sendMessage(jid, { image: buffer, caption, ...opts }, { quoted });

  conn.sendVideo = (jid, buffer, caption = '', quoted, opts = {}) =>
    conn.sendMessage(jid, { video: buffer, caption, ...opts }, { quoted });

  conn.sendAudio = (jid, buffer, quoted, opts = {}) =>
    conn.sendMessage(jid, { audio: buffer, mimetype: 'audio/mpeg', ...opts }, { quoted });

  conn.sendPTT = (jid, buffer, quoted, opts = {}) =>
    conn.sendMessage(jid, { audio: buffer, mimetype: 'audio/ogg; codecs=opus', ptt: true, ...opts }, { quoted });

  conn.sendSticker = (jid, buffer, quoted, opts = {}) =>
    conn.sendMessage(jid, { sticker: buffer, ...opts }, { quoted });

  conn.sendDocument = (jid, buffer, mimetype, fileName, caption = '', quoted, opts = {}) =>
    conn.sendMessage(jid, { document: buffer, mimetype, fileName, caption, ...opts }, { quoted });
  ...
  conn.sendButton = async (jid, buttons, quoted, opts = {}) => {
    // Baileys v7 – native flow buttons
    const btns = buttons.map(([text, id]) => ({
      name: 'quick_reply',
